...
//STALLIE, IDLEIE, TRNIE, and URSTIE are all enabled by default and are required
     ...
     #if   (__PIC32MX__)
        #define USBEnableInterrupts() {IEC1bits.USBIE = 1;IPC7CLR=0x001F0000;IPC7SET=0x00050000; INTEnableSystemMultiVectoredInt(); INTEnableInterrupts();}
      // fix error: 'IPC11CLR' undeclared , error: 'IPC11SET' undeclared in f250, IPC1CLR and IPC10CLR
      // for the f250 The USB Interrupt priority is IPC7<20:18> and the sub priority is IPC7<17:16>
      // try IPC7CLR=0x001F0000 and IPC7SET=0x00050000
     #else
